...
const i18n = {
  ru: {
    ...
    copied: 'Текст скопирован', openSource: 'Открыть первоисточник', rules: 'Правила', demoMode: 'Демо-режим', externalBrowser: 'Внешний браузер', offlineDemo: 'Оффлайн-демо', dataError: 'Ошибка данных', startupError: 'Ошибка запуска', university: 'Университет', government: 'Госслужбы', chooseDate: 'Выберите дату'
  },
  en: {
    ...
    copied: 'Copied', openSource: 'Open primary source', rules: 'Rules', demoMode: 'Demo mode', externalBrowser: 'External browser', offlineDemo: 'Offline demo', dataError: 'Invalid data', startupError: 'Startup error', university: 'University', government: 'Government', chooseDate: 'Select date'
  },
  zh: {
    eyebrow: 'MAX · 关爱大众', tagline: '为国际学生生成抵俄后首批行政手续的个性化办理路线。',
    profileTitle: '生成您的办理路线', profileLead: '本MVP面向18岁以上、首次赴俄就读圣彼得堡国立经济大学（СПбГЭУ）全日制课程的国际学生。',
    arrival: '入境俄罗斯日期', housing: '当前居住地点', dorm: 'СПбГЭУ 学生宿舍', private: '校外私人住址',
    over90: '我计划在俄罗斯停留超过90天', belarus: '我是白俄罗斯公民', visaRequired: '赴俄学习需要签证', visaExpiry: '签证有效期至', regExpiry: '移民登记有效期至（如已办理）', build: '生成办理路线',
    eventTitle: '您的情况有变化吗？', eventLead: '请选择发生的情况：重新入境、入住酒店/医院、搬家或更换护照。', eventDate: '事件日期', recalc: '重新计算路线', clearEvent: '清除事件', eventNote: 'СПбГЭУ要求发生上述情况后，在1个工作日内联系签证与登记部门。',
    sourcesTitle: '信息来源', sourcesLead: '每条规则都链接到官方原始来源，并标注核验日期。', aboutTitle: '关于本产品', about1: '“学生路线（СтудМаршрут）”不替代俄罗斯“Госуслуги”国家服务门户，而是把大学与政府手续整合为一条个性化办理顺序。', about2: 'MVP目前未与俄罗斯内务部（МВД）或“Госуслуги”实时集成，使用预先核验的规则和示例用户数据。', share: '在MAX中分享路线', disclaimer: '本工具不构成政府部门的正式决定，也不能替代主管机构的咨询。',
    navRoute: '路线', navEvent: '变更', navSources: '来源', navAbout: '关于', tasks: '项任务', done: '已完成', urgent: '紧急', source: '来源', due: '截止', noDue: '官方来源未注明具体期限', saved: '路线已保存在本设备', shareText: '我已通过MAX中的“学生路线（СтудМаршрут）”生成抵达后的首个行政手续路线。',
    copied: '文本已复制', openSource: '打开官方来源', rules: '规则核验', demoMode: '演示模式', externalBrowser: '外部浏览器', offlineDemo: '离线演示', dataError: '数据有误', startupError: '启动错误', university: '大学', government: '政府手续', chooseDate: '选择日期'
  }
};
...
function updateDateDisplays() {
  for (const id of dateFieldIds) {
    ...
    display.textContent = input.value ? formatDateISO(input.value) : t('chooseDate');
...
function renderRulesBadge() {
  $('rulesBadge').textContent = `${t('rules')}: ${state.rulesVersion ? formatDateISO(state.rulesVersion) : '…'}`;
}
...
function taskHtml(task) {
  ...
  const due = task.due_date ? `${t('due')} ${formatDateISO(task.due_date)}` : t('noDue');
...
function renderSources() {
  $('sourceList').innerHTML = state.sources.map((s) => `<div class="source"><strong>${escapeHtml(localizedField(s, 'title'))}</strong><small>${escapeHtml(localizedField(s, 'scope'))} · ${escapeHtml(formatDateISO(s.checked_at))}</small><button class="source-link" data-url="${escapeAttr(s.url)}">${t('openSource')}</button></div>`).join('');
...
async function init() {
  ...
  $('clearEvent').addEventListener('click', async () => { $('eventDate').value = ''; updateDateDisplays(); await buildRoute(); });
